...
function PersonalInfo() {
  ...
  const handleChangePassword = async () => {
    try {
      const passwordURL = "https://qltd01.cfapps.us10-001.hana.ondemand.com/user/password";
      const payload = {
      ...
      const response = await axios.put(passwordURL, payload, {
        headers: {
          'Authorization': `Bearer ${accessToken}`
        }
      });
      ...
      if (response.status === 200) {
        ...
        toast.error("Failed to change password. Please check your current password.", { position: toast.POSITION.TOP_RIGHT, autoClose: 2000 });
      }
    } catch (error) {
      console.error('Password Change Error:', error);
      toast.error("Failed to change password. Please try again later.", { position: toast.POSITION.TOP_RIGHT, autoClose: 2000 });
    }
  };
                ...
                <div className="section">
                  ...
                  <Form>
                    ...
                    <Form style={{ marginTop: "10px" }}>
                      ...
                      <div className="d-flex justify-content-end">
                        {/* Button "Save" */}
                        <Button variant="outline-success" className="change-password-btn" onClick={handleChangePassword}>
                          Change Password
                        </Button>
                        <ToastContainer />
                      </div>
