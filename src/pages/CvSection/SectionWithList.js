...
function SectionWithList({ iconName ,title, type, sectionData, oneRow = false, handleUpdateData }) {
		...
		<div className='section'>
			...
			{sectionData.map((item, idx) => (
				<table className='tbl' key={idx}>
					<tr>
						<td className='cv-ipt' contentEditable rowSpan={oneRow ? 1 : 2}>{item[0]}</td>
					...
					{!oneRow &&
						<tr>
							<td className='cv-ipt' contentEditable>{item[2]}</td>
						</tr>
					}
					<BtnAddItem idx={idx} />
